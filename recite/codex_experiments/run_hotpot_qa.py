...
from codex_experiments.utils import predict_call, recite_predict_call, cot_predict_call, contextual_predict_call
from codex_experiments.utils import hotpotqa_answer_postprocess, get_majority_voting
import tasks_and_mixtures.few_shot_hotpot_qa_tasks as few_shot_hotpot_qa_tasks  # pylint: disable=unused-import
from tasks_and_mixtures.prompts import HOTPOT_RECITE_PROMPT, HOTPOT_QA_PROMPT
from tasks_and_mixtures.prompts_for_hotpot_qa import HOTPOTQA_FIXED_DIRECT_PROMPT
from tasks_and_mixtures.prompts_for_hotpot_qa import HOTPOTQA_FIXED_RECITE_PROMPT
from tasks_and_mixtures.prompts_for_hotpot_qa import HOTPOTQA_FIXED_COT_PROMPT
...
def hq_input_processing_fn(inputs, vocab,
                           exemplar_splitter="\n\n\n",
                           sentence_splitter="\n\n",
  ...
  if inference_scheme == "direct":
    decoder_inputs = exemplar_splitter.join([HOTPOTQA_FIXED_DIRECT_PROMPT] + new_examples)
    return decoder_inputs
  elif inference_scheme == "cot":
    new_examples = sentence_splitter.join(new_examples)
    decoder_inputs = exemplar_splitter.join([HOTPOTQA_FIXED_COT_PROMPT, new_examples])
    return decoder_inputs
  elif inference_scheme == "recite":
    qa_inputs = exemplar_splitter.join([HOTPOTQA_FIXED_DIRECT_PROMPT, sentence_splitter.join(new_examples)])
    new_examples = sentence_splitter.join([new_examples[0], HOTPOT_RECITE_PROMPT, new_examples[1]])
    ...
    recite_inputs = exemplar_splitter.join([HOTPOTQA_FIXED_RECITE_PROMPT, new_examples])
    return recite_inputs, qa_inputs
  else:
    raise ValueError("Unknown inference scheme: %s" % inference_scheme)


def make_majority_voting(multiple_results):
  return list(map(
      functools.partial(get_majority_voting,
                        post_process_fn=hotpotqa_answer_postprocess),
      multiple_results))
...
def direct_answer_fn(dataset, task,
  ...
  for batch_id, ex in dataset.as_numpy_iterator():
    ...
    predictions = [hotpotqa_answer_postprocess(p) for p in predictions]
...
def cot_answer_fn(dataset, task,
                  batch_size,
                  self_consistency_k,
                  exemplar_splitter="\n\n\n",
                  sentence_splitter="\n\n",
                  verbose=False):
  """Answer a question with chain-of-thought.

  Args:
    dataset: A tf.data.Dataset.
    task: A seqio.Task.
    batch_size: Batch size for evaluation.
    self_consistency_k: The number of self-consistency paths.
    exemplar_splitter: The string used to split the exemplars.
    sentence_splitter: The string used to split the sentences within an exemplar.
    verbose: Whether to print the input and output.
  ...
  for batch_id, ex in dataset.as_numpy_iterator():
    ...
    cot_predictions = cot_predict_call(qa_inputs, self_consistency_k=self_consistency_k)

    predictions = make_majority_voting(cot_predictions)

    for example_id in range(batch_id.shape[0]):
      if verbose:
        logging.info("** QA INPUTS %d **" % batch_id[example_id])
        logging.info(qa_inputs[example_id])
        logging.info("** COT OUTPUTS %d - path 0 **" % batch_id[example_id])
        logging.info(cot_predictions[example_id][0].split(sentence_splitter)[0])
        logging.info("** FINAL OUTPUTS %d **" % batch_id[example_id])
        logging.info(predictions[example_id])
...
def recite_and_answer_fn(dataset, task,
  ...
  for batch_id, ex in dataset.as_numpy_iterator():
    ...
    recite_predictions = recite_predict_call(recite_inputs, self_consistency_k,
                                             sentence_splitter=exemplar_splitter)

    def recite_postprocess_fn(texts):
      def clean_text(text):
        text = text.split("\nQuestion:")[0].strip()
        text = ("\n" + text).replace("\nAnswer ", "\n")
        return text
      return [clean_text(_) for _ in texts]

    recite_predictions = list(map(recite_postprocess_fn, recite_predictions))
    ...
    qa_predictions = contextual_predict_call(qa_inputs, recite_predictions, qa_prompt=HOTPOT_QA_PROMPT)
    predictions = make_majority_voting(qa_predictions)
...
def main(args):
  ...
  seqio_task = f"learning_to_recite:hotpot_qa_open_{args.num_shots}shot_fixed_lm_gpt2"

  if args.num_shots != 4:
    raise ValueError("Only 4-shot is supported for now.")
  ...
  elif args.inference_scheme == "cot":
    predict_fn = functools.partial(
        cot_answer_fn,
        batch_size=args.batch_size,
        self_consistency_k=args.self_consistency_k,
